...
TODO: 语法上支持像调用method一样，调用匿名函数
如：需要支持 a.b() // a是类，其中b是匿名函数类型的field

MethodCallExpression 改名为 MemberCallExpression
然后在 fix阶段，解析 b是method 还是一个filed（它的类型为匿名函数变量类型）

TODO: 系统性测试
匿名函数捕获自由变量


TODO: 给 field赋值的时候，需要比对 func类型，如果类型不匹配，则需要报错
入：
```
...
#### 8. 匿名函数变量在没有初始化的时候，如何保证 调用他们的合法性
...
#### 10. 关于 fn 定义返回值的优化，简化，也包括在 typeof的时候，更命名的时候保持一致


#### 11. 为什么 在 global块中不支持定义 匿名函数变量，

1. 需要给出编译报错
2. 全局匿名函数完全可以有  普通函数代替，不然会让可读性降低
