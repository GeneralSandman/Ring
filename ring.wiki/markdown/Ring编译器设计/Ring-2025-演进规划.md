...
// args 中老是占用数据，会造成干扰，不太好调试数据
// 如果可以disable就好了
// 如果 命令的 fn main() {} 就不初始化对应的参数
...
#### 8. string array gc heap_size 会变成负数


#### 9. 这里写法在 会造成垃圾泄漏，应该补齐一个 pop 语句
...
#### 10. RVM_Array 中 需要改为 RVM_String** RVM_ClassObject** RVM_Clsure**
...
#### 13. auto-test 能够kill掉耗时长的任务

#### 14. 是否深度复制，需要取决于语义，需要 deep_copy 语句
...
#### 18. 扫清 generate.cpp 中 关于 `// TODO: 编译报错` 相关的问题
...
AssignExpression 应该归为语句，而不是表达式
...
#### 26. for-range 赋值进行详细的语法/语义检查  ✅

新增一个函数，判断一个 TypeSpecifier 是 一个 TypeSecifier的 下一个纬度的类型

#### 27. for-range 中，array 参数 进行丰富 TODO: 系统性测试
...
1. break/continue 外层没有找到对应 for 语句怎么办
...
#### 30. in range 调整为 = range  ✅

#### 先保障 stw 垃圾回收算法的正确性和可调试性
