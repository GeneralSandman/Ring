...
bug:
1. 给 field 复制之后, printfInfo不会改变, 查看 ./test/008-class/class-method-004.ring
2. 通过 class object literal 初始化 class-object 之后, 调用 method会报错 ./test/008-class/class-method-005.ring (new class object literal的时候, 没有函数表的指针, 导致内存访问错误)
