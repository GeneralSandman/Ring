...
6. Array & ClassObject
7. Array & String  ✅ 2023-10-31
8. Test array bool(global/local/argument/return)  ✅ 2023-10-31
...
var bool[] bool_array_0;

var int index;
var bool value;
for(index, value = range bool_array_0) {
    fmt::println_int(index);
}
```
...
FOR_RANGE_CALL (push a value about iter)

FOR_RANGE_LOOP (
    get value of array[iter]:
        if iter >= array.length -> jump to finish-label
        else            -> run block

    assign iter,array[iter] -> index,value
    add iter)
...
FOR_RANGE_FINISH(pop iter from stack)
