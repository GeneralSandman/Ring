...
1. 如何打印 object的地址, 这样能够区分object是否做了深度copy 
2. 重构一些函数 ✅
3. 通过工具 可以将 ring vmcode summary 导出 markdown doc ✅
5. 函数 返回 array string object 去完成深度copy
...
0. 实现一个简单的垃圾回收机制 mark&swap
1. native 函数 添加 vm::grbage_collect
2. Add native function vm::heap_size
3. 计算 heap_size 的方式需要重新计算

bug  free object 遗留问题
```
 /*
         * 这里有个遗留问题
         *
