 ...
 */
int do_simple_kevent(struct kevent64_s* kev, uint64_t expected) {
    int             ret;
    ...
    /* did it work? */
    elapsed_usecs = (after.tv_sec - before.tv_sec) * (1000 * 1000) + (after.tv_usec - before.tv_usec);
    delta_usecs   = abs((long long)(elapsed_usecs - (expected)));
...
void test_absolute_kevent(int time, int scale) {
    struct timeval    tv;
...
void test_oneshot_kevent(int time, int scale) {
    int               ret;
    uint64_t          expected = 0;
    struct kevent64_s kev;
...
void test_repeating_kevent(int usec) {
    struct kevent64_s kev;
    ...
    if (kev.data > expected_pops + (expected_pops / 20) || kev.data < expected_pops - (expected_pops / 20)) {
...
// 源码： https://opensource.apple.com/source/xnu/xnu-1504.7.4/tools/tests/kqueue_tests/kqueue_timer_tests.c.auto.html
// g++ -std=c++11 -o kqueue_timer_tests ./kqueue_timer_tests.cpp  && ./kqueue_timer_tests ./tmp.txt
